...
@interface MHPagedRequester ()

@property (nonatomic, readwrite, getter = isFetchingNewPage) BOOL fetchingNewPage;

@property (strong, nonatomic) id<MHRequesterPagedResponse> latestResponse;
...
@implementation MHPagedRequester
...
- (void)refreshNextPage
{
    ...
    fetchedModel.then(^(id<MHRequesterPagedResponse> response) {
        NSAssert([response conformsToProtocol:@protocol(MHRequesterPagedResponse)], @"MHPagedRequester requires the model to conform to MHRequesterPagedResponse");
